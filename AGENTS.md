...
| 테스트 작성 (구현보다 **먼저**) | 아키텍처 결정을 임의로 바꾸기 |
...
**구현 코드보다 테스트를 먼저 쓴다.** 스펙을 읽고 나서 바로 구현에 들어가지 마라.

1. **Red** — 스펙이 요구하는 동작을 검증하는 테스트를 먼저 쓴다. `pnpm test` 를 돌려 **실패하는 것을 눈으로 확인**한다. 통과해버리면 그 테스트는 아무것도 검증하지 않는 것이다
2. **Green** — 테스트를 통과시키는 **최소한의** 구현을 한다
3. **Refactor** — 테스트가 초록인 상태를 유지하면서 정리한다

테스트를 나중에 붙이면 "구현이 하는 일"을 그대로 베낀 테스트가 나온다. 그건 버그를 못 잡는다.

### 무엇에 어떤 테스트를 붙이나

| 이런 걸 만들면 | 이 테스트를 함께 만든다 | 어디에 |
|---|---|---|
| 순수 함수·유틸 (금액 계산·집계·포맷, 날짜 경계, 조건 분기) | Vitest 단위 테스트 **필수** | 대상 파일 옆 `*.test.ts` |
| 커스텀 훅, 스토어 액션 | Vitest **필수** | 대상 파일 옆 `*.test.ts` |
| 입력·제출·토글 등 상호작용이 있는 컴포넌트 | Testing Library 컴포넌트 테스트 **필수** | 대상 파일 옆 `*.test.tsx` |
| 사용자가 실제로 밟는 동선 (거래 입력 → 목록 반영 등) | Playwright e2e 스펙 **필수** (작성만, 실행은 CI) | `e2e/*.spec.ts` |
| props 만 받아 그리는 프레젠테이션 컴포넌트, 스타일 | 테스트하지 않는다 | — |

**"테스트 가능한데 안 썼다"는 미완성이다.** 스펙에 테스트 요구가 안 적혀 있어도 위 표에 해당하면 쓴다 — 이건 "스펙에 없는 것은 하지 마라"의 예외다. 반대로 표의 마지막 줄(프레젠테이션·스타일)에 테스트를 붙이는 것은 범위 이탈이다.

테스트를 쓸 수 없다고 판단했으면 **왜 못 쓰는지를 최종 보고에 적는다.** 조용히 건너뛰지 마라.

상세 작성법은 `.claude/skills/testing-conventions/SKILL.md` 를 읽는다.
...
`pnpm e2e` 는 **네가 할 수 없다.** 샌드박스가 브라우저 실행을 막아 Chrome 이 SIGABRT 로 죽는다. 시도하지 마라. 브라우저에서만 드러나는 회귀는 `e2e/` 의 Playwright 스펙으로 고정하고(스펙 파일 작성은 네가 한다), 실제 실행은 CI 의 e2e 잡이 한다.
...
무엇에 무엇을 붙이는지는 위 "작업 순서 — 테스트가 먼저다" 의 표를 따른다. 여기는 작성 규칙이다.

- 라이브러리 동작(zustand 가 저장하는지, Query 가 캐시하는지)은 테스트하지 않는다
- 테스트는 **동작**에 붙인다. 내부 state 이름·클래스명·호출 횟수에 붙은 테스트는 리팩토링을 막는 부채다
