...
- **테스트 누락**: 순수 함수·훅·스토어 액션·상호작용 컴포넌트가 추가·변경됐는데 대응 테스트가 없다. 판단 기준은 `AGENTS.md` 의 "작업 순서 — 테스트가 먼저다" 표다. 프레젠테이션 컴포넌트와 스타일은 해당 없다
- **해피 패스만 테스트**: 경계값(월말, 0원, 음수, 빈 배열)이나 분기의 반대편이 검증되지 않았다
