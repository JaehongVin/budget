...
Codex gpt-5.6-terra (effort medium)                ← 구현
        ↓  diff
diff 확인 → code-reviewer (+ ui-qa)                 ← 총괄 (Opus)
...
## 완료 조건
- `pnpm verify` 통과
- `src/features/transactions/model/types.test.ts` 의 테스트가 모두 통과
```
...
- **스킬 파일 경로.** Codex는 `AGENTS.md` 는 자동으로 읽지만 `.claude/skills/**` 는 안 읽는다. 필요한 스킬은 경로를 찍어 "읽고 따르라"고 써라.

## Codex 샌드박스의 한계

Codex 는 `workspace-write` 샌드박스에서 돈다. 파일 쓰기와 `pnpm` 실행은 되지만 **브라우저를 띄우지 못한다.** Chrome 을 실행하면 SIGABRT 로 죽는다.

따라서 아래는 스펙의 완료 조건에 넣어도 Codex 가 수행할 수 없다. **총괄이 직접 하거나 `ui-qa` 에게 맡긴다.**

- `pnpm e2e` (Playwright)
- 스크린샷, 레이아웃 실측, 콘솔 에러 확인

스펙에는 "무엇이 만족되어야 하는지"를 수치로 적어두고, 실측은 위임 후 총괄이 한다. Codex 에게는 코드 레벨 검증(`pnpm verify`, `pnpm build`)까지만 요구한다.
...
2. `code-reviewer` 를 돌린다. `.tsx`/`.css` 가 바뀌었으면 `ui-qa` 도 병렬로.
